...
class TestWorkflowSpec(WorkflowSpec):
  def __init__(self):
    ...
    a2 = UserInput(self, 'task_a2', args=["sid", "mildrid"])
    a1.connect(a2)
    end = Simple(self, 'End')
    a2.connect(end)
...
class WorkflowFunctionalTests(unittest.TestCase):
  ...
  def testValidJsonSpec(self):
    jsonSerializedSpec = self.spec.serialize(JSONSerializer())
    self.assertTrue(json.loads(jsonSerializedSpec))

    self.assertIn('nose', jsonSerializedSpec)
    self.assertIn('face', jsonSerializedSpec)
    self.assertIn('mildrid', jsonSerializedSpec)

    jsonDeSerializedSpec = JSONSerializer().deserialize_workflow_spec(jsonSerializedSpec)
    self.assertTrue(jsonDeSerializedSpec)

  def testValidJsonWorkflow(self):
    jsonSerializedWf = self.wf.serialize(JSONSerializer())
    self.assertTrue(json.loads(jsonSerializedWf))
